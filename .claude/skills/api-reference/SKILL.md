...
Authoritative content: read `.kiro/skills/api-reference/SKILL.md`.

That file is shared with the Kiro IDE so there is one copy to maintain. Read it
now before continuing.

**Adding or changing an endpoint?** Update `.kiro/skills/api-reference/SKILL.md`
in the same change — a stale endpoint table is how agents invent routes that
don't exist.
