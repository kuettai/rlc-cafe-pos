  ...
  function renderWalkup(){
    ...
    modal.querySelectorAll('.pos-add-btn').forEach(b=>b.onclick=()=>{
      ...
      if(existing){ existing.qty++; }
        ...
        cart.push({
          name:b.dataset.mname, menuItemId:b.dataset.mid, price:+b.dataset.mp, qty:1, variant:null,
          ...
          basePrice:item.basePrice||item.price||+b.dataset.mp,
        });
      }
      updateCart();
    });
    modal.querySelectorAll('.pos-variant-btn').forEach(b=>b.onclick=()=>{
      ...
      if(existing){ existing.qty++; }
        ...
        name:item.name, menuItemId:b.dataset.mid, price:variantPrice, qty:1, variant:b.dataset.v, selectedVariants:sv,
        category:item.category||'DRINK',
        celebrationEligible:item.celebrationEligible===true,
        basePrice:basePrice,
      }); }
