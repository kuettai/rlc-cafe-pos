...
(function () {
  ...
  function renderEntry(e) {
    ...
    return `
      <article style="padding:12px 14px;border-bottom:1px solid var(--cream-dark)">
        <header style="display:flex;align-items:baseline;gap:10px;flex-wrap:wrap;margin-bottom:6px">
          <span style="font-weight:700;color:var(--band);background:var(--cream);padding:2px 8px;border-radius:6px;font-size:.85rem">${esc(e.version || '?')}</span>
          <span style="color:var(--text-light);font-size:.8rem">${esc(e.date || '')}</span>
        </header>
  ...
  async function open() {
    ...
    overlay.innerHTML = `
      <div style="background:var(--card);color:var(--ink);border-radius:14px;max-width:640px;width:100%;max-height:70vh;display:flex;flex-direction:column;box-shadow:0 10px 40px rgba(0,0,0,.25);overflow:hidden">
        <header style="display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid var(--cream-dark);background:var(--cream)">
          <h2 style="margin:0;font-size:1.1rem;color:var(--band)">📋 Changelog</h2>
          <button type="button" data-changelog-close aria-label="Close" style="background:none;border:none;font-size:1.4rem;cursor:pointer;padding:4px 10px;color:var(--text-light);line-height:1">✕</button>
        </header>
        <div data-changelog-body style="overflow-y:auto;flex:1;min-height:80px">
          <div style="padding:24px;text-align:center;color:var(--text-light)">Loading…</div>
        </div>
        <footer style="padding:10px 18px;border-top:1px solid var(--cream-dark);background:var(--cream);font-size:.75rem;color:var(--text-light);text-align:right">Press Esc or click outside to close</footer>
      </div>`;
    ...
    try {
      ...
      if (!entries.length) {
        body.innerHTML = '<div style="padding:24px;text-align:center;color:var(--text-light)">No entries.</div>';
        return;
      }
      body.innerHTML = entries.map(renderEntry).join('');
    } catch (err) {
      body.innerHTML = `<div style="padding:24px;text-align:center;color:var(--stale-ink)">Failed to load changelog: ${esc(err && err.message || 'error')}</div>`;
    }
  }
