...
- ☐ FOOD is never discounted **by `STAFF` or `PREORDER`** — those two classes are
  DRINK-only, and `CELEBRATION` is eligible-DRINK-only. **`PASTOR` and `NEWCOMER`
  DO discount FOOD**: they price **both** categories at RM0, expressed as an
  allowlist per class (`CLASS_CATEGORIES`, `backend/src/lib/pricing.ts:170`).
  **It is an allowlist and not a negation, on purpose.** Two properties are
  ...
  and `Record<CustomerClass, …>` is **exhaustive**, so adding a fifth customer
  class without deciding its categories is a compile error rather than a silent
  ...
  overstate takings on a pastor's or newcomer's food order, and a discount
