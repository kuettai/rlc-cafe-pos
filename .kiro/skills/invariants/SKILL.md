...
- ☐ FOOD is never discounted **by `STAFF` or `PREORDER`** — those two classes are
  DRINK-only, and `CELEBRATION` is eligible-DRINK-only. **`PASTOR`, `NEWCOMER` and
  `BLESSING` DO discount FOOD**: they price **both** categories at RM0, expressed
  as an allowlist per class (`CLASS_CATEGORIES`,
  `backend/src/lib/pricing.ts:182`) — the all-category side of that allowlist now
  has **three** members, and `BLESSING` is the one whose *whole definition* is
  "everything, RM0", so a new menu category must be added to it or the waiver
  silently stops being full.
  **It is an allowlist and not a negation, on purpose.** Two properties are
  load-bearing: it **fails closed** — `priceLine` coerces the category with
  `String(menu.category || '')`, so a malformed menu record with no category
  matches nothing, where a `category !== 'DRINK'` test would hand it out free —
  and `Record<CustomerClass, …>` is **exhaustive**, so adding a **sixth** customer
  class (there are five today) without deciding its categories is a compile error
  rather than a silent
  DRINK-only default. Do not "simplify" either property away. An earlier version
  of this list said "FOOD is never discounted" flatly; that is now false, and a
  report, export or tile that assumes food lines always carry full price will
  overstate takings on a pastor's, newcomer's or blessed food order, and a discount
  breakdown that counts only drink lines under-reports what was given away. One
  helper reads the allowlist — `classAppliesToCategory` — and both gates
  (submission-time and approve-time) call it; never inline the test at a gate
  (see `pricing-rules`).
...
- ☐ **`parseCustomerClass()` never accepts a SYSTEM-ONLY customer class.** Its
  ...
  **The test of that rule is WHO DECIDES, never the size of the discount.**
  `BLESSING` (v1.83) was the first class added under this rule and was correctly
  **admitted**: it zeroes an entire order across both categories — a *larger*
  waiver than `PREORDER` — yet a human at the till selects it and is recorded in
  `approvedBy`, so it must be expressible in a request. It is not an exception to
  this invariant; it is the demonstration that the invariant is drawn on the right
  axis. If you find yourself keeping a class out of `parseCustomerClass` because it
  discounts *too much*, you have applied the wrong test: ask instead whether the
  server derives it from stored state (keep it out — a request must not be able to
  assert a server-derived fact) or whether a person chooses it (let it in, and make
  sure the choice is attributable).
...
- ☐ **A VALIDATION THAT LIVES ONLY IN THE BROWSER IS NOT A RESTRICTION THE API
  HAS.** The second direction of the rule above, and the easier one to miss,
  because the screen *looks* like it enforces the rule. Found while adding
  `PUT /api/admin/display/slides/{id}`: a slide's `startDate`/`expiryDate` had to
  be `YYYY-MM-DD` with expiry on or after start, both checks were in
  `admin-display.js` only, and `POST /api/admin/display/slides` accepted a
  free-text date and an inverted range. Hardening just the new edit route would
  have been theatre. Both paths now call one `slideDateRejection()`
  (`backend/src/routes/admin.ts:53`, from `:1178` create and `:1207` edit), which
  **changes the behaviour of the existing POST** — it now 400s on input it used
  to store. That is the correct direction: `routes/display.ts` compares those
  dates lexicographically, so a bad value was never rejected, just silently
  mis-compared, and the slide never appeared on the TV. When adding a restriction
  to an edit path, grep the **frontend** too — if that is where the rule has been
  living, the create endpoint is already a bypass. And see **A TEST CAN PIN A
  DEFECT IN PLACE** below: the suite here asserted `201` for an inverted range.
...
- ☐ **A TEST CAN PIN A DEFECT IN PLACE, AND THAT IS WORSE THAN NO TEST.** The
  ...
  **A second instance, and it is not a one-off.** `backend/tests/admin-misc.test.ts`
  carried `it('does NOT validate the date FORMAT, nor that expiry follows start')`
  for `POST /api/admin/display/slides`, asserting `201` for both a free-text date
  and an inverted range. Its comment described the resulting production failure
  accurately and at length — and still asserted the defect was the expected
  result, so adding the validator turned the suite red. It has been rewritten as
  three positive tests (format rejected, inverted range rejected, `start ===
  expiry` accepted) that each assert **no write happened**. Note what the two
  instances have in common: a characterisation test that *understands* the hazard
  is still a test that fails when the hazard is fixed. Write the comment, then
  make the assertion the behaviour you want — not the one you have.
...
- ☐ **The `sw.js` `fetch` listener returns early for any non-`http(s)` scheme.**
  `if (!e.request.url.startsWith('http')) return;` is the first line of the
  handler. Falling through to `e.respondWith(...)` for a `chrome-extension://`,
  `chrome://` or `data:` request means `cache.put` rejects — the Cache API only
  accepts `http`/`https` schemes — which rejects the `respondWith` promise and
  makes the browser fail a request the page never issued. Returning without
  calling `respondWith` hands it back to the browser's default handling, which is
  the only correct answer for a scheme we do not cache. Found live in v1.80.0:
  an installed Chrome extension was generating a steady stream of
  `chrome-extension://` fetches through the service worker, each one throwing
  inside the cache write and filling the console. Any new branch added to that
  handler sits **after** the guard, never before it.
...
- ☐ **Animate `transform` / `opacity`, never a layout property.** `width`,
  ...
  must return nothing.
- ☐ **A cross-fade between two images uses two stacked elements, and the swap is
  driven by the image's `load` event — never by a `setTimeout` that guesses the
  CSS duration.** One element cannot cross-fade with itself: fading it out,
  swapping `src`, and fading it back in dips through whatever is behind it, and
  the new `src` only starts decoding *as it becomes visible*, so the fade-in can
  reveal a half-decoded frame. Found in v1.80.0 on the TV display, which ran
  exactly that pattern on a 1000 ms timer hard-coded to match a `1s` CSS
  transition — two numbers in two files that had to agree, with nothing to catch
  it when one changed. The fix is the pattern to copy: `.promo-layer` A and B both
  absolutely positioned and transitioning `opacity` only, a throwaway
  `new Image()` preloading the incoming URL, and the class swap performed in
  **both** `onload` and `onerror` — a preload that never resolves must not be
  able to freeze the slideshow. (v1.80.0's `onerror` branch still activates the
  broken URL rather than skipping to the next slide; not a regression, and
  recorded as a follow-up in `docs/update-20260923.md`.)
  `clearPromoLayers()` in `frontend/js/display.js` is the one place
  that resets both layers — the old code cleared a single `src` from two separate
  branches. Why it matters: this is a 1080p TV in a public foyer, and the old
  swap flashed the dark `#111` panel between every slide.
...
- ☐ **A status must carry a non-colour channel as well as a colour one — a word
  ...
  **Reusing the audited fallback and leaning on the word is the sanctioned cheap
  path.** A new variant does **not** have to earn a hue. `BLESSING` (v1.83) is the
  first `discountType` to ship on the fallback grey `other` pill *deliberately*
  rather than taking a seventh colour (`frontend/js/pos.js:1096-1102`, where the
  reasoning is recorded at the call site). Measured: Blessing vs the Pastor pill is
  **ΔE76 4.2** — they genuinely do read similarly — with text contrast **5.35:1**,
  and the word "Blessing" carries the non-colour channel the rule above demands. A
  seventh hue would have needed ΔE against all six existing badges in normal *and*
  deutan vision, which is real work and was not done; grey plus the word is
  honest, passes contrast, and does not claim a distinction the palette has not
  earned. Do not "finish the set" by inventing a hex for it — if you want Blessing
  to have its own colour, do the six-way measurement first.
